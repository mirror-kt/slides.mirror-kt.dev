...
const formatter = computed(
    // biome-ignore lint/suspicious/noExplicitAny: The type information on the Slidev side is rough, so I cannot strictly define the types.
...
function usePloy(number = 16) {
    function getPoints(): Range[] {
        ...
        function applyOverflow(random: number, overflow: number) {
            ...
            return rng() < disturbChance
                ? random + (rng() - 0.5) * disturb
                : random;
        }
